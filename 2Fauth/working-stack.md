...
docker run --rm -it 2fauth/2fauth id
sudo chmod -R 777 /root/docker/2fauth/2fauth_data
sudo chown -R 1000:1000 /root/docker/2fauth/2fauth_data
sudo chown -R 0:0 /root/docker/2fauth/2fauth_data
docker run --rm -it 2fauth/2fauth id
ls -l /root/docker/2fauth/2fauth_data/database/database.sqlite
chmod -R 777 /root/docker/2fauth/2fauth_data
ls -l /root/docker/2fauth/2fauth_data/database/database.sqlite
sqlite3 /2fauth/database/database.sqlite
```

```sh
sudo apt install php -y
```
#### Copy the key generated by the command and update the - APP_KEY=base64:
```sh
...
services:
  2fauth:
    image: 2fauth/2fauth
    container_name: 2fauth3
    volumes:
      - /root/docker/2fauth/2fauth_data:/2fauth
    ports:
      - 8092:8000/tcp #Change this to a port you wish to use
    environment:
      ...
      - APP_KEY=base64:ZpUPtRFOaW1tTAnYy2zAt8YlQjPlWK/fEKe74TyDSSE=
      # This variable must match your installation's external address.
      # Webauthn won't work otherwise.
      - APP_URL=http://10.0.100.10:8092 #if you change the port, make sure to change it here as well, or if via https change to https://domainname
      ...
      - ASSET_URL=http://10.0.100.10:8092
